...
3. **Kurtosis tail correction** — BTC has excess kurtosis ~7-12 (fat tails vs GBM). For deep ITM/OTM brackets (z-score > 2.0), a tail adjustment prevents overconfident 99%+ probabilities. All probabilities are clamped to [0.02, 0.98] to reflect irreducible jump/crash risk
...
5. **Basis signal** — Perpetual futures vs spot basis indicates directional sentiment
6. **Funding rate signal** — Positive funding (longs pay shorts) signals bullish market structure
7. **Deribit DVOL** — Market-implied volatility from the BTC volatility index, preferred over realized vol when available
