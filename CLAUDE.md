...
### Testing
- `just test` — 209 Rust tests
- `just test-python` — 373 Python tests
- `just test-all` — both (582 total)
...
just test           # Run Rust tests (209)
just test-python    # Run Python tests (373)
just test-all       # Run both (582 total)
...
| `rust/src/crypto_asset.rs` | CryptoAsset enum (BTC/ETH/SOL/XRP/DOGE) |
...
Phases 0-9.0 are complete. Phase 5 added: per-strategy analytics & Brier scoring (5.1), calibration dashboard (5.2), P&L attribution with model_components JSONB (5.3), reconciliation loop (5.4), clock discipline (5.5), dead-letter handling (5.6), integration tests (5.7), per-feed health scoring (5.8). Phase 6.1: parameter sweep framework, daily settlement summary, collector enhancements (crypto_ticks, settlement aggregation). Phase 7: calibration agent & prediction feedback loop — fixes signal_id/outcome/latency_ms data plumbing, adds calibration daemon with 6 hourly jobs, confidence-scaled order sizing, book-walking fill estimation, VWAP microstructure signal, evaluator hot-reload, price momentum (7.3a), volume surge detection (7.3c), OI delta tracking (7.3d), edge trajectory tracking (7.5). Phase 8: advanced backtesting & adaptive calibration — foundation fixes (8.0a-e), transaction costs (8.2), advanced metrics (8.3), crypto threshold sweep (8.1), multi-signal eval (8.4), parallel sweep (8.5), replay engine with source ablation (8.6), comprehensive tests (8.7). Phase 9.0: Grafana observability — decision_log + feed_health_log tables, 4 auto-provisioned dashboards, 5 alert rules (Discord), Rust + Python decision trace instrumentation. Phase 10.1: Crypto model profitability fixes (fill price, negative spread, risk/reward guard). Phase 11.0-11.4: Bloomberg terminal dashboard — 6-page terminal UI (MAIN, SGNL, EXEC, ANAL, RISK, WEAT) with Chart.js, SSE, IBM Plex Mono amber theme. Phase 12.2: Crypto model accuracy fixes — orderbook delta price complement bug, N(d2) tail risk floor/ceiling (PROB_FLOOR=0.02/PROB_CEILING=0.98 + kurtosis correction), max edge filter (0.25 default), lowered thresholds (min_edge=0.03, min_kelly=0.02, min_confidence=0.40), expanded sweep grid with max_edge dimension. Phase 12.3: Crypto calibration overhaul — BINARY_VOL_MULTIPLIER=2.5 (jump risk premium), PROB_FLOOR=0.10/PROB_CEILING=0.90 (tighter clamps), vol fallback in Python evaluator (DEFAULT_VOL=0.60), wider observation lookback (5→15 min), crypto_max_edge raised 0.25→0.35, crypto_max_market_disagreement 0.25→0.35. Phase 13: Multi-asset crypto support — CryptoAsset enum (BTC/ETH/SOL/XRP/DOGE), per-asset AssetConfig (vol/kurtosis tuning), CryptoStateRegistry (per-asset CryptoState), multi-product Coinbase/Binance feeds, asset-aware evaluator dispatch, per-asset config enable flags, per-asset feed health tracking. Phase 14: Crypto model calibration overhaul — tail compression function (identity [0.25,0.75], 0.20 factor), 4 missing guards ported to Python (market_disagreement, max_edge, risk_reward, directional_atm), vol multiplier 2.0→2.5 Rust, PROB_CEILING 0.95→0.80 Rust / 0.90→0.80 Python, min_edge 0.02→0.08 Rust / 0.06→0.08 Python, min_kelly 0.01→0.04, entry_max_minutes 59→20, cooldown 15→300s, max_signals_per_hour=20, max_market_disagreement 0.35→0.25, max_edge 0.35→0.25. Phase 15 (fixes 4-11): batch system-status CTE query, dashboard pool 3→10, Coinbase subscription ACK validation, evaluator decision_log backpressure, low-volume feed health thresholds (SOL/XRP/DOGE), debounce 500→250ms, per-asset trading thresholds (min_edge/min_kelly/max_edge/cooldown per asset), Postgres tuning in docker-compose. See `docs/build-plans/phase-8-backtesting.md`.
